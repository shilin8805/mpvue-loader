...
const pagesInfoMap = Object.create(null)
...
function cachePageInfo (resourcePath, ...arg) {
  pagesInfoMap[resourcePath] = Object.assign({}, pagesInfoMap[resourcePath], ...arg)
}

function getPageInfo (resourcePath) {
  return pagesInfoMap[resourcePath] || {}
}
...
function getFiltersOutputSrc (context, file, fileExt) {
  const filePath = `/${resolveSrc(context, file)}.${fileExt.customScript}`
...
function getBabelrc (src) {
  ...
  const curBabelConfig = path.resolve('./babel.config.js')
  if (fs.existsSync(curBabelRc)) {
    return curBabelRc
  } else if (fs.existsSync(curBabelConfig)) {
    return curBabelConfig
  }
  return ''
}
