...
function compileMP (content, mpOptioins) {
  ...
  if (isApp) {
    ...
    // 解析全局组件的路径
    const components = {}
    resolveImportSrc(importsMap, globalComps, components, resolve, context, options.context, babelrc).then(() => {
...
function resolveImportSrc (importsMap, globalComps, components, resolveFn, context, projectRoot, babelrc) {
  return Promise.all(Object.keys(importsMap).map(k => {
    return new Promise((resolve, reject) => {
      resolveFn(context, importsMap[k], (err, realSrc) => {
        ...
        let babelResult
        try {
          babelResult = babel.transform(importFileSource, { extends: babelrc, plugins: parseGlobalComponents })
        } catch (e) {}
        if (babelResult) {
    ...
    return resolveSrc(globalComps, components, resolveFn, context, projectRoot)
  })
}
