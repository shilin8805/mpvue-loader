...
function compileMP (content, mpOptioins) {
  ...
  if (isApp) {
    ...
    // 解析全局组件的路径
    const components = {}
    const currentRecursiveLevel = 0
    const maxRecursiveLevel = 3
    resolveImportSrc(importsMap, globalComps, components, resolve, context, options.context, babelrc, mpOptioins.fileExt, currentRecursiveLevel, maxRecursiveLevel).then(() => {
...
function resolveImportSrc (importsMap, globalComps, components, resolveFn, context, projectRoot, babelrc, fileExt, currentRecursiveLevel, maxRecursiveLevel) {
  return Promise.all(Object.keys(importsMap).map(k => {
    return new Promise((resolve, reject) => {
      resolveFn(context, importsMap[k], (err, realSrc) => {
        ...
        let babelResult
        try {
          babelResult = babel.transform(importFileSource, { extends: babelrc, plugins: [parseConfig, parseGlobalComponents] })
        } catch (e) {}
        if (babelResult) {
          const { metadata } = babelResult
          const { globalComponents, importsMap: importsMapInner } = metadata
          ...
          if (importsMapInner && Object.keys(importsMapInner).length > 0 && currentRecursiveLevel <= maxRecursiveLevel) {
            currentRecursiveLevel++
            return resolveImportSrc(importsMapInner, globalComps, components, resolveFn, context, projectRoot, babelrc, fileExt, currentRecursiveLevel, maxRecursiveLevel).then(() => {
              resolve()
            })
          } else {
            resolve()
